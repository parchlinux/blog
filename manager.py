...
def postman(ptype, option, post):
    if option == "edit":
        ...
        if ptype == "blog":
            ...
            blog_post_path = os.path.join("content", "blog", category, filename)
            ...
            faq_post_path = os.path.join("content", "faq", post)
        ...
        if ptype == "blog":
            filename, category = post.split("@")
            delete_post(
                os.path.join("content", "blog", category, filename), docs_dir="b"
            )
        else:
            delete_post(os.path.join("content", "faq", post), docs_dir="faq")
